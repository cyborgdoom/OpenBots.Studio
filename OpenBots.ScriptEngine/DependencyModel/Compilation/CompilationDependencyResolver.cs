...
using System.IO;
using System.Linq;
using OpenBots.DependencyModel.Context;
using OpenBots.DependencyModel.Environment;
using OpenBots.DependencyModel.Logging;
using OpenBots.DependencyModel.Process;
    ...
    public class CompilationDependencyResolver
    {
        ...
        public IEnumerable<CompilationDependency> GetDependencies(string targetDirectory, IEnumerable<string> scriptFiles, bool enableScriptNugetReferences, string defaultTargetFramework = "net46")
        {
                ...
                defaultTargetFramework.StartsWith("net5", StringComparison.InvariantCultureIgnoreCase))
            {
                ...
                result.Add(new CompilationDependency("OpenBots.Default.Dependencies", "99.0", compilationreferences.Select(cr => cr.Path).ToArray(), Array.Empty<string>()));
            }
